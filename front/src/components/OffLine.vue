<script setup>
import {computed, nextTick, onBeforeUnmount, onMounted, reactive, ref, watch} from 'vue';
import {API_BASE_URL} from "@/api/config.js";
import axios from "axios";

const repositoryURL = ref('');
const loading = ref(false);
// 右侧的项目信息
const projectName = ref('');
const modelName = ref('');
const fileSelected = ref(null);
const Proname = ref('');
const isReadyReport = ref(false);

// const detectProject = ref({
//   Pname: '香苞漫画',
//   Pmd5: 'ab36de45be4b3ccbd3f069f358a9e4c5',
//   Pprob: '0.875',
//   Pres: '1',
//   Pmodel: '快速检测',
//   Ppackage: 'com.mhapp',
//   Plabel: 'sex'
// })

const detectProject = ref(null)

const user = ref({
  UFacePath: '/path/to/user/image.png',
  Uname: 'User Name',
  Uposition: 'User Position'
});

const projects = ref([]);
// Variables for project information, initialized as needed

// 左侧的项目信息
const project = ref({
  Proname: '',
  Pfilepath: '',
  Pmodel: 'ReGVD'
});

// 中间的项目信息
const qrCode = ref({
  Proname: '',
  Pimage: '',
  Pmodel: '',
})

// 右侧的项目信息
const localProject = ref({
  Proname: '',
  Pmodel: '',
  Pfile: ''
});

const canSubmit1 = computed(() => {
  return project.value.Proname.trim() && project.value.Pfilepath.trim() && project.value.Pmodel;
});

const canSubmit2 = computed(() => {
  return qrCode.value.Proname.trim() && qrCode.value.Pimage && qrCode.value.Pmodel;
});

const canSubmit3 = computed(() => {
  return localProject.value.Pfile && localProject.value.Pmodel;
});

async function submitProject() {
  showProgress(true);
  console.log('Submitting project:', project.value);
  // 构造 FormData 对象
  const formData = new FormData();
  formData.append('url', project.value.Pfilepath);
  formData.append('model', project.value.Pmodel);  // 添加模型信息

  // 发送 POST 请求给后端
  await fetch(`${API_BASE_URL}/upload/url`, {
    method: 'POST',
    body: formData,
  })
      .then(response => response.json())
      .then(data => {
        console.log('Upload response:', data);
        // 可以根据后端返回的数据进行后续处理
        isReadyCheck.value = true;
        md5.value = data.md5;
        console.log('URL上传成功，返回md5:', md5.value)
      })
      .catch(error => {
        console.error('Error uploading file:', error);
      });
  showProgress(false);
}

function showProgress(isShow) {
  loading.value = isShow;
  if (isShow) {
    document.getElementById("contu").style.setProperty("padding-top", "0.8rem", "important");
  } else {
    document.getElementById("contu").style.setProperty("padding-top", "1.625rem", "important");
  }

}

async function submitQRProject() {
  showProgress(true);
  console.log('Submitting QR project:', qrCode.value);
  // 构造 FormData 对象
  const formData = new FormData();
  formData.append('image', qrCode.value.Pimage);
  formData.append('model', qrCode.value.Pmodel);  // 添加模型信息

  // 发送 POST 请求给后端
  await fetch(`${API_BASE_URL}/upload/qr`, {
    method: 'POST',
    body: formData,
  })
      .then(response => response.json())
      .then(data => {
        console.log('Upload response:', data);
        // 可以根据后端返回的数据进行后续处理
        isReadyCheck.value = true;
        md5.value = data.md5;
        console.log('二维码上传成功，返回md5:', md5.value)
      })
      .catch(error => {
        console.error('Error uploading file:', error);
      });
  showProgress(false);
}


function fetchProjects() {
  // API call to fetch projects (pseudo-code)
  projects.value = [
    {
      Pname: 'Project A', Ptime: '2021-01-01T00:00:00Z', Pvul: 5, Pfix: 3
    },
    // Add more projects as needed
  ];
}

const handleFileChange = (event) => {
  if (event.target.files[0]) {
    if (!event.target.files[0].type.startsWith('application/vnd.android.package-archive')) {
      alert("请上传APK文件");
      return;
    }
  }
  localProject.value.Pfile = event.target.files[0];
};

async function submitLocalProject() {
  showProgress(true);
  let formData = new FormData();
  formData.append('file', localProject.value.Pfile);
  formData.append('model', localProject.value.Pmodel);  // 添加模型信息
  try {
    let response = await fetch(`${API_BASE_URL}/upload`, {
      method: 'POST',
      body: formData
    });
    if (response.ok) {
      console.log('File uploaded successfully');
      // window.location.href = '/home/project';
      isReadyCheck.value = true;
      const data = await response.json();
      md5.value = data.md5;

      console.log('返回md5:', md5.value);
      const infoResponse = await axios.get(`/off_data?md5=${md5.value}`);
      const infoData = infoResponse.data;
      projects.value = infoData.data.map((item, index) => ({
        Pname: item.App_Name,
        Pmd5: item.MD5,
        Pprob: item.prob === '0' ? '尚未检测' : item.prob,
        Pres: item.res === '-1' ? '尚未检测' : item.res,
        Pmodel: item.model,
        Ppackage: item.Package_Name,
        Plabel: item.Label
      }));
      detectProject.value = projects.value[0];
      saveProject(detectProject.value);
    } else {
      alert("文件上传失败");
      console.log('File upload failed');
    }
  } catch (error) {
    console.log('Error:', error);
  }
  showProgress(false);
}


const handleQRChange = (event) => {
  if (event.target.files[0]) {
    if (!event.target.files[0].type.startsWith('image/')) {
      alert("请上传图片文件");
      return;
    }
  }
  qrCode.value.Pimage = event.target.files[0];
};

function postProject() {
  console.log('Posting project:', project.value);
}

const maxHeight = ref(0);

// 将三个卡片的高度设置为三者中的最大高度
const setMaxHeight = () => {
  nextTick(() => {
    const linkCard = document.querySelector('.link-card');
    const qrCodeCard = document.querySelector('.qr-code-card');
    const localCard = document.querySelector('.local-card');

    if (linkCard && qrCodeCard && localCard) {
      const heights = [linkCard.offsetHeight, qrCodeCard.offsetHeight, localCard.offsetHeight];
      maxHeight.value = Math.max(...heights);
    }
    linkCard.style.height = maxHeight.value + 'px';
    qrCodeCard.style.height = maxHeight.value + 'px';
    localCard.style.height = maxHeight.value + 'px';
  });
};

// --------------进度条----------------
const progress = ref(0);
const progressText = ref('0%');
const progressBarWidth = ref('0%');
const intervalId = ref(null);
const isReadyCheck = ref(false);
const md5 = ref('');

async function simulateLoading() {
  progress.value = 0;
  intervalId.value = setInterval(() => {
    const speed = Math.random() * 2;
    progress.value += speed;
    progressBarWidth.value = `${progress.value}%`;
    progressText.value = `${Math.round(progress.value)}%`;
    if (progress.value >= 90) {
      clearInterval(intervalId.value);
      progressBarWidth.value = `90%`;
      progressText.value = `90%`;
    }
  }, 1500); // 每隔500毫秒更新一次
}

async function startDetect() {
  if (md5.value) {
    try {
      await axios.post('/detect', {
        md5: md5.value,
      }).then(res => {
        if (res.status === 200) {
          if (progress.value < 90) {
            clearInterval(intervalId.value);
          }
          progress.value = 100;
          progressBarWidth.value = `100%`;
          progressText.value = `100%`;
          // isReadyCheck.value = false;
        } else {
          progress.value = 0;
          progressBarWidth.value = `0%`;
          progressText.value = `0%`;
          isReadyCheck.value = true;
          throw new Error('Service not 200');
          // console.log('Error:', res);
        }
        fetchCatalogue();
      });
      // 更新
      const infoResponse = await axios.get(`/off_data?md5=${md5.value}`);
      const infoData = infoResponse.data;
      projects.value = infoData.data.map((item, index) => ({
        Pname: item.App_Name,
        Pmd5: item.MD5,
        Pprob: item.prob === '0' ? '尚未检测' : item.prob,
        Pres: item.res === '-1' ? '尚未检测' : item.res,
        Pmodel: item.model,
        Ppackage: item.Package_Name,
        Plabel: item.Label
      }));
      detectProject.value = projects.value[0];
      isReadyReport.value = true;
    } catch (error) {
      // 处理网络错误或其他异常
      progress.value = 0;
      progressBarWidth.value = '0%';
      progressText.value = '0%';
      isReadyCheck.value = true;
      clearInterval(intervalId.value);
      alert("检测失败，请稍后再试");
      console.error('Network or other error:', error);
    }
  }
}


import {marked} from 'marked';
import router from "@/router/index.js";

const leftChats = reactive([
  {name: "Package Tracer", message: "", parsedMessage: computed(() => marked("Hello, I am Package Tracer."))},
  {name: "Content Analizer", message: "", parsedMessage: computed(() => marked("Hello, I am Content Analyzer."))},
  {
    name: "Certificate Inspector",
    message: "",
    parsedMessage: computed(() => marked("Hello, I am Certificate Inspector."))
  }
]);

const rightChats = reactive([
  {name: "Icon Analizer", message: "", parsedMessage: computed(() => marked("Hello, I am Icon Analyzer."))},
  {
    name: "Sensitive Info Analizer",
    message: "",
    parsedMessage: computed(() => marked("Hello, I am Sensitive Info Analyzer."))
  },
  {
    name: "Relationship Analizer",
    message: "",
    parsedMessage: computed(() => marked("Hello, I am Relationship Analyzer."))
  }
]);

const updateParsedMessages = () => {
  leftChats.forEach(chat => {
    chat.parsedMessage = computed(() => marked(chat.message));
  });

  rightChats.forEach(chat => {
    chat.parsedMessage = computed(() => marked(chat.message));
  });
};

const fetchCatalogue = async () => {
  try {
    const response = await axios.get(`/api/chat-messages?md5=${md5.value}`);
    const data = response.data;

    leftChats[0].message = data.leftChats[0]; // Package Tracer
    leftChats[1].message = data.rightChats[1]; // Content Analizer
    leftChats[2].message = data.leftChats[2]; // Certificate Inspector
    rightChats[0].message = data.rightChats[0]; // Icon Analizer
    rightChats[1].message = data.leftChats[1]; // Sensitive Info Analizer
    rightChats[2].message = data.rightChats[2]; // Relationship Analizer

    updateParsedMessages();
  } catch (error) {
    console.error('Error fetching chat messages:', error);
  }
};

const submitAction = async (action, projectMD5) => {
  console.log(`Action: ${action}, Project ID: ${projectMD5}`);
  if (action === 'lookInside') {
    router.push({
      name: 'offanalyze',
      params: {md5: projectMD5},
    });
  }
  if (action === 'startCheck') {
    try {
      const response = await axios.post('/detect', { md5: projectMD5 });
      console.log('Start checking project:', response.data); // Log success or response data
      // ElMessage({
      //   showClose: true,
      //   plain: true,
      //   type: 'success',
      //   message: '成功开始检测APK！',
      // });
      alert('成功检测APK！');
    } catch (error) {
      console.error('Error starting checking project:', error);
      // ElMessage({
      //   showClose: true,
      //   plain: true,
      //   type: 'error',
      //   message: '检测APK失败！',
      // });
      alert('检测APK失败！');
    }
  }
  if (action === 'deleteProject') {
    try {
      // Send a POST request to delete project by MD5
      const response = await axios.post('/delete_apk', { md5: projectMD5 });
      console.log('Delete project:', response.data); // Log success or response data
      // Optionally, refresh data after deletion
      await fetchData(currentPage.value); // Refresh the project list
      // ElMessage({
      //   showClose: true,
      //   plain: true,
      //   type: 'success',
      //   message: '成功删除APK信息！',
      // });
      alert('成功删除APK信息！');
    } catch (error) {
      console.error('Error deleting project:', error);
      // ElMessage({
      //   showClose: true,
      //   plain: true,
      //   type: 'error',
      //   message: '删除APK信息失败！',
      // });
      alert('删除APK信息失败！');
    }
  }
  if (action === 'dynamicSim') {
    await router.push({
      name: 'realTimeDetect',
      params: {package_name: projectMD5},
    });
  }
};

const saveProject = (project) => {
  localStorage.setItem('lastProject', JSON.stringify(project));
};

const getLastProject = () => {
  const project = localStorage.getItem('lastProject');
  return project ? JSON.parse(project) : null;
};


onMounted(async () => {
  if (getLastProject()!==null) {
    detectProject.value = getLastProject();
    isReadyCheck.value = true;
    md5.value = detectProject.value.Pmd5;
  }
  fetchProjects();
  // setMaxHeight();
});

onBeforeUnmount(() => {
  clearInterval(intervalId.value);
});
</script>


<template>
  <transition name="fade">
    <div class="loader-container" v-if="loading">
      <div class="loader-bar"></div>
    </div>
  </transition>
  <div class="container-xxl flex-grow-1 container-p-y" id="contu">
    <div class="row">

      <!-- Local Upload -->
      <div class="col-md-5">
        <div class="card mb-4 local-card">
          <h5 class="card-header">本地</h5>
          <div class="text-center">
            <i class='bx bx-code-block' style="font-size: 100px;"></i>
          </div>
          <div class="card-body demo-vertical-spacing demo-only-element">
            <form @submit.prevent="submitLocalProject">

              <!-- 本地上传部分 -->
              <div class="row mb-3">
                <label class="col-sm-3 col-form-label">上传apk</label>
                <div class="col-sm-9">
                  <div class="input-group">
                    <label class="input-group-text" for="inputGroupFile01">
                      <i class='bx bxs-file-archive'></i>
                    </label>
                    <input type="file" class="form-control" accept=".apk,application/apk" @change="handleFileChange">
                  </div>
                </div>
              </div>

              <div class="row mb-3">
                <label class="col-sm-3 col-form-label">选择模型</label>
                <div class="col-sm-9">
                  <div class="input-group">
                    <!-- 本地上传部分 -->
                    <select v-model="localProject.Pmodel" class="form-select">
                      <option value="精准检测">精准检测</option>
                      <option value="快速检测">快速检测</option>
                    </select>
                    <button class="btn btn-outline-primary" style="margin: 0" name="submit_button"
                            value="file" type="submit" id="uploadBtn" :disabled="!canSubmit3">上传！
                    </button>
                  </div>
                </div>
              </div>
            </form>
          </div>
        </div>
      </div>

      <div class="col-md-7">
        <div class="card">
          <h5 class="card-header">检测结果</h5>
          <div v-if="detectProject!==null" class="card-body card-body-full">
            <div class="project-card">
              <div class="">
                <div class="project-details">
                  <h4 class="card-title mb-4">{{ detectProject.Pname }}</h4>
                  <p class="card-text">
                    <div class="detail-item">
                      <strong>MD5:</strong> {{ detectProject.Pmd5 }}
                    </div>
                    <div class="detail-item">
                      <strong>包名:</strong> {{ detectProject.Ppackage }}
                    </div>
                    <div class="detail-item">
                      <strong>风险程度:</strong> {{ detectProject.Pprob }}
                    </div>
                    <div class="detail-item">
                      <strong>检测结果:</strong>
                      <span v-if="detectProject.Pres === '1'" class="badge bg-label-danger">{{ detectProject.Plabel }}</span>
                      <span v-else-if="detectProject.Pprob === '0'" class="badge bg-label-info">未检测</span>
                      <span v-else class="badge bg-label-success">White</span>
                    </div>
                    <div class="detail-item">
                      <strong>模型:</strong> {{ detectProject.Pmodel }}
                    </div>
                  </p>
                </div>
                <div class="action-buttons" style="text-align: right">
                  <button @click="submitAction('lookInside', detectProject.Pmd5)" class="btn btn-primary"
                          :disabled="detectProject.Pprob === '0' || !isReadyReport">查看详情</button>
                </div>
              </div>
            </div>
          </div>
          <div v-else class="card-body text-center card-body-empty">
            <p>检测项目为空！请上传并检测项目。</p>
          </div>
        </div>
      </div>




      <!--      下面的内容     -->
      <div class="col-md-12">
        <!-- Contextual Classes -->
        <div class="card">
          <div class="d-flex align-items-center align-content-center justify-content-between">
            <h5 class="card-header w-25">检测进度 <span class="text-primary">{{ progressText }}</span></h5>
            <div class="px-4">
              <button class="btn-primary btn rounded w-15" :disabled="!isReadyCheck"
                      @click=" simulateLoading(); startDetect();">
                开始检测
              </button>
            </div>
          </div>

          <div class="card-body">
            <div class="progress">
              <div class="progress-bar progress-bar-striped progress-bar-animated bg-primary"
                   role="progressbar" :style="{ width: progressBarWidth }" :aria-valuenow="progress"
                   aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          </div>

          <div class="card-body" style="display: block">
            <div class="row" style="height: 100%; width: 100%">
              <div class="col-md-6">
                <div class="chat-left" v-for="(item, index) in leftChats" :key="'left-' + index">
                  <div class="role-container">
                    <img :src="`/role${index + 1}.jpg`" alt="role image" class="role-image">
                    <div class="role-name">{{ item.name }}</div>
                  </div>
                  <div class="speech-bubble" v-html="item.parsedMessage"></div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="chat-right" v-for="(item, index) in rightChats" :key="'right-' + index">
                  <div class="speech-bubble" v-html="item.parsedMessage"></div>
                  <div class="role-container">
                    <img :src="`/role${index + 4}.jpg`" alt="role image" class="role-image">
                    <div class="role-name">{{ item.name }}</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!--/ Contextual Classes -->
      </div>
    </div>
  </div>

</template>


<style scoped>
.report-loading-text {
  justify-content: center;
  align-items: center;
  font-size: 18px;
  font-weight: bold;
}

.fade-enter-active, .fade-leave-active {
  transition: opacity 0.5s;
}

.fade-enter-from, .fade-leave-to {
  opacity: 0;
}

.loader-container {
  width: 100%;
  height: 4px; /* 加载条的高度 */
  margin-top: 0.8%;
//background-color: white; //border: 1px solid #ccc; border-radius: 2px; overflow: hidden; /* 隐藏超出容器部分的加载条 */ position: relative;
}

.loader-bar {
  width: 10%; /* 加载条的初始宽度 */
  height: 100%;
  background-color: rgb(96, 91, 255); /* 加载条颜色 */
  position: absolute;
  animation: slide 4s ease-in-out infinite; /* 动画，持续时间2秒，线性速度，无限循环 */
}

@keyframes slide {
  from {
    left: -10%; /* 从左边界开始，开始时加载条不可见 */
  }
  to {
    left: 100%; /* 移动到容器的右端，结束时加载条不可见 */
  }
}

.row {
  display: flex;
  justify-content: space-between;
}

.chat-left, .chat-right {
  display: flex;
  align-items: flex-start;
  margin-bottom: 10px;
}

.chat-left {
  justify-content: flex-start;
}

.chat-right {
  justify-content: flex-end;
}

.role-container {
  display: flex;
  flex-direction: column;
  align-items: center;
  margin-right: 10px;
}

.chat-right .role-container {
  margin-left: 10px;
  margin-right: 0;
}

.role-image {
  width: 50px;
  height: 50px;
}

.speech-bubble {
  background-color: #f0f0f0;
  border-radius: 10px;
  padding: 10px;
  width: 100%;
  word-wrap: break-word;
}

.chat-right .speech-bubble {
  background-color: #d0e6ff;
}

.role-name {
  font-size: 14px;
  color: #333;
  margin-top: 5px;
  text-align: center;
}
.card-body-empty {
  display: flex;
  align-items: center;
  justify-content: center;
  height: 257px; /* 固定高度，与有内容的部分一致 */
}

.card-body-full {
  height: 257px; /* 固定高度，与有内容的部分一致 */
}

.detail-item {
  margin-bottom: 0.25rem; /* 添加每项之间的底部间距 */
}
</style>